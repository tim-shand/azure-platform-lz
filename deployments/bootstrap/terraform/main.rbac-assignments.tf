...
resource "azurerm_role_assignment" "rbac_sp_custom" {
  ...
  role_definition_name = azurerm_role_definition.custom_role_iac_deploy.name # Required to deploy all resource types in tenant. 
...
resource "azurerm_role_assignment" "rbac_cu_cfg" {
  scope                = azurerm_management_group.core.id #data.azurerm_management_group.tenant_root.id # Assign at root management group.
  role_definition_name = "App Configuration Data Owner"   # Required to read/write App Config key values. 
