...
output "bootstrap_iac_sa" {
  ...
  value       = module.iac_sa["bootstrap"].storage_account_name # Keyed by the deployment_stack key.
}

output "bootstrap_iac_cn" {
  description = "The name of the Storage Account Container for the bootstrap backend."
  value       = azurerm_storage_container.iac_cn["bootstrap"].name # Keyed by the flattened stack key.
}
