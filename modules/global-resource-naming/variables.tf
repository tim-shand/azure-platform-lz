...
variable "project" {
  ...
  nullable    = true
  validation {
    condition     = var.project == null || (length(var.project) >= 3 && length(var.project) <= 10)
...
variable "environment" {
  ...
  nullable    = true
  validation {
    condition     = var.environment == null || (contains(["dev", "tst", "stg", "prd", "plz", "alz"], var.environment) && length(var.environment) == 3)
